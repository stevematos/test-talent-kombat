...
from utils.general import get_value_exceed_max_characters, merge_list
    ...
    for i in range(len_actions):
        ...
        if i < len_first_player_actions:
            ...
            history_messages.append(
                f'{first_player.name} {attack_first_player["message"]}'
            )

            life_points_second_player = second_player.receive_damage(
                attack_first_player["damage"]
            )
        ...
        if i < len_second_player_actions:
            ...
            history_messages.append(
                f'{second_player.name} {attack_second_player["message"]}'
            )

            life_points_first_player = first_player.receive_damage(
                attack_second_player["damage"]
            )
...
def start_game(player_actions: dict[str, dict[str, list]]) -> list[str]:
    ...
    player_1_tuple_actions = merge_list(
        player_1_actions.get("movimientos"), player_1_actions.get("golpes")
    )
    player_2_tuple_actions = merge_list(
        player_2_actions.get("movimientos"), player_2_actions.get("golpes")
    )
    ...
    (
        ...
        second_player_actions,
    ) = get_order_player(
        player_1, player_2, player_1_tuple_actions, player_2_tuple_actions
    )

    history_messages = _execute_actions(
        first_player, second_player, first_player_actions, second_player_actions
    )
